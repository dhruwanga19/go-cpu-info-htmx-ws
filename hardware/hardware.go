...
func GetSystemSection() (string, error) {
	...
	html = html + "<tr><td>Operating System:</td> <td><i class='fa fa-brands fa-microsoft'></i> " + runTimeOS + "</td></tr>"
	html = html + "<tr><td>Platform:</td><td> <i class='fa fa-brands fa-windows'></i> " + hostStat.Platform + "</td></tr>"
...
func GetDiskSection() (string, error) {
	diskStat, err := disk.Partitions(true)
	if err != nil {
		return "", err
	}
	...
	html = html + "<tr><td>File system:</td><td>" + diskStat[0].Fstype + "</td></tr>"
	html = html + "<tr><td>Partitions:</td><td>" + strconv.Itoa(len(diskStat)) + "</td></tr>"
	html = html + "<tr><td>Total disk space:</td><td>" + strconv.FormatUint(usageStat.Total/gigabyteDiv, 10) + " GB</td></tr>"
	html = html + "<tr><td>Free disk space:</td><td>" + strconv.FormatUint(usageStat.Free/gigabyteDiv, 10) + " GB</td></tr>"
	html = html + "<tr><td>Percentage disk space usage:</td><td>" + strconv.FormatFloat(usageStat.UsedPercent, 'f', 2, 64) + "%</td></tr>"
